...
public class GridManager : MonoBehaviour {
  ...
  void Awake() {
    ...
    state = State.Loaded;
        if (PlayerPrefs.HasKey("HighScore"))  // check if we already save It before
        {
    ...
    private void SaveBoard()
    {
        for (int y = 0; y < rows; y++)
        {
            for (int x = 0; x < cols; x++)
            {
                ...
                if (obj == noTile)
                {
                    PlayerPrefs.SetInt(worldPosition.ToString(), 999);

                    //Debug.Log(worldPosition + " " + obj);
                    continue;
                }
                PlayerPrefs.SetInt(worldPosition.ToString(), obj.GetComponent<Tile>().power - 1);
                PlayerPrefs.SetInt("Score", points);
                //Debug.Log(worldPosition + " " + obj.GetComponent<Tile>().power);
